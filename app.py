...
def on_new_message(data):
    ...
    if bot_message["is_bot"] == True:
        DB.session.add(
            models.Chat(
                "bot",
                botcommand.bot_command_parse(
                    bot_message["bot_command"], bot_message["message"]
                ),
            )
        )
        DB.session.commit()
